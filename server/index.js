...
app.post("/api/admin/templates",auth,adminOnly("templates.manage"),async(req,res)=>{
 ...
 try{const {rows}=await pool.query("insert into templates(slug,name,description,version,price_cents,preview_url,package_url,checksum_sha256,metadata) values($1,$2,$3,$4,$5,$6,$7,$8,$9) returning *",[slug.trim().toLowerCase(),name.trim(),description?.trim()||null,version,Math.max(0,Number(priceCents)||0),previewUrl||null,packageUrl||null,checksumSha256||null,JSON.stringify(metadata)]);await audit(req,"admin.template.create","template",rows[0].id);res.status(201).json(rows[0]);}catch(e){if(e.code==="23505")return res.status(409).json({error:"Template slug already exists"});res.status(500).json({error:"Unable to create template"});}});
